...
import sys
from utils import extract_planning, content_to_json, print_response, print_log_cost, load_accumulated_cost, save_accumulated_cost, read_python_files, format_json_data
import copy
...
doc_desc = ""
if os.path.exists(f'{args.output_repo_dir}'):
    doc_desc = "You will be provided with a baseline code implementation. Your task is to analyze the gap between the baseline and the paper, and plan the necessary modifications."
...
baseline_content = ""
if os.path.exists(f'{args.output_repo_dir}'):
    baseline_content_dict = read_python_files(args.output_repo_dir)
    baseline_content = format_json_data(baseline_content_dict)
...
def get_write_msg(todo_file_name, todo_file_desc):
    ...
    if len(baseline_content) > 0:
        draft_desc = f"Conduct a Logic Analysis to assist in **refining/modifying** the baseline code in '{todo_file_name}', which is intended for '{todo_file_desc}'. Identify which parts of the baseline can be reused, which need modification, and what is missing."
        baseline_msg = f"""## Baseline Code Reference
        
The following files exist in the baseline:
```
{baseline_content}
```
        """
...
for todo_file_name in tqdm(todo_file_lst):
    ...
    if " " in todo_file_name:
        # Split by space and take the first part that looks like a file path
        parts = todo_file_name.split(" ")
        for part in parts:
            # Simple heuristic: acts like a file path if it has extensions or slashes, 
            # and isn't a numbering "1."
