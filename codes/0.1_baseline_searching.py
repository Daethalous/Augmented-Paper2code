...
def calculate_directory_similarity(dir1, dir2):
    """
    Calculate similarity between two directories based on file structure and content.
    Returns a dictionary with structure_similarity and content_similarity.
    """
    ...
    if not files1 or not files2:
        return {"structure_similarity": 0.0, "content_similarity": 0.0, "common_files_count": 0, "total_files_union": 0}

    # Structure Similarity (Jaccard Index)
    ...
    # Content Similarity (for common files)
    content_sim_scores = []
    for rel_path in intersection:
        ...
        try:
            ...
            with open(path1, 'r', encoding='utf-8', errors='ignore') as f1, \
                ...
                content_sim_scores.append(matcher.ratio())
        except Exception:
            # Skip binary or unreadable files
            pass

    avg_content_sim = sum(content_sim_scores) / len(content_sim_scores) if content_sim_scores else 0.0

    return {
        "structure_similarity": structure_sim, 
        "content_similarity": avg_content_sim,
        "common_files_count": len(intersection),
        "total_files_union": len(union)
    }
...
def main(args):
    ...
    if target_repo:
        ...
        # --- Added: Similarity Check against Gold Repo ---
        # Assuming gold repos are stored in a standard location relative to the project root or provided via args
        # We try to locate the 'data/gold_repos' directory based on common project structure
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        # Adjust if 'codes' is not direct child of root, but here inputs suggest structure.
        # Structure: root/codes/0.1..., root/data/gold_repos/
        
        gold_repos_base = os.path.join(project_root, "data", "paper2code", "gold_repos")
        
        # Try to find matching gold repo. 
        # Case 1: args.paper_name matches folder name exactly
        gold_repo_path = None
        if args.paper_name:
             candidate = os.path.join(gold_repos_base, args.paper_name)
             if os.path.exists(candidate) and os.path.isdir(candidate):
                 gold_repo_path = candidate
        
        # Case 2: If finding by paper_name fails, maybe check if any folder in gold_repo 
        # is a substring or close match. (Skipped for now to keep it simple and safe)
        
        if gold_repo_path:
            print(f"[Evaluation] Comparing cloned baseline with Gold Repo: {gold_repo_path}")
            sim_metrics = calculate_directory_similarity(args.output_repo_dir, gold_repo_path)
            
            print("="*40)
            print(f"BASELINE VS GOLD REPO SIMILARITY REPORT")
            print("="*40)
            print(f"Paper: {args.paper_name}")
            print(f"Cloned Repo: {target_repo}")
            print(f"Gold Repo Path: {gold_repo_path}")
            print(f"Structure Similarity (Jaccard): {sim_metrics['structure_similarity']:.4f}")
            print(f"Content Similarity (Avg Ratio):  {sim_metrics['content_similarity']:.4f}")
            print(f"Common Files: {sim_metrics['common_files_count']} / {sim_metrics['total_files_union']}")
            print("="*40)
            
            # Save metrics
            eval_path = os.path.join(args.output_dir, "preprocess_artifacts", "baseline_similarity.json")
            with open(eval_path, 'w', encoding='utf-8') as f:
                json.dump(sim_metrics, f, indent=2)
        else:
            print(f"[Evaluation] Gold repo not found for '{args.paper_name}' in {gold_repos_base}. Skipping comparison.")
